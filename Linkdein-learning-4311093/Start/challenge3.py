...
class Book():
    ...
    def __str__(self) -> str:
        return f"<Book: {self.title} by {self.author} {self.pages} {self.cover} {self.price}"
    ...
    def __getattr__(self, attr):
        if attr == "adjustedprice" and self.antique==True:
            if self.cover==CoverType.PAPERBACK:
                return self.price+8
            else: 
                return self.price+10
        elif attr=="adjustedprice" and self.antique==False:
            if self.cover==CoverType.PAPERBACK:
                return self.price-2
            else:
                return self.price
...
books = [
    ...
    Book("Brave New World", "Aldous Huxley", 311, 
         CoverType.PAPERBACK, True, 32.50),
    Book("Crime and Punishment", "Fyodor Dostoevsky", 492, 
         CoverType.HARD, False, 19.75),
    Book("Moby Dick", "Herman Melville", 427, 
         CoverType.PAPERBACK, True, 22.95),
    Book("A Christmas Carol", "Charles Dickens", 66, 
         CoverType.HARD, False, 31.95),
